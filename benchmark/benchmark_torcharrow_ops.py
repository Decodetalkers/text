...
def run_torchtext2_ops():
    ...
    with Timer("Running torchtext2's GPT2BPE tokenizer"):
        tokenized_text = tokenizer(text_list)

    with Timer("Running torchtext2's vocab query"):
        token_ids = vocab(tokenized_text)

    with Timer("Running torchtext2's add tokens operation (string)"):
        add_bos_str(tokenized_text)
        add_eos_str(tokenized_text)

    with Timer("Running torchtext2's add tokens operation (int)"):
        add_bos_int(token_ids)
        add_eos_int(token_ids)

    with Timer("Running torchtext2's to tensor conversion"):
