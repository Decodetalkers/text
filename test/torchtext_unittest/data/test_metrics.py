...
class TestUtils(TorchtextTestCase):
    def test_bleu_score(self) -> None:
        ...
        refs = [[["My", "full", "pytorch", "test"], ["Different"]]]
        self.assertEqual(bleu_score(candidate, refs, max_n=2, weights=[0.5, 0.5]), 0.5066641)

        # Multi-sentence corpus
        candidate = [["My", "full", "pytorch", "test"], ["Another", "Sentence"]]
        refs = [[["My", "full", "pytorch", "test"], ["Completely", "Different"]], [["No", "Match"]]]
        ...
        refs = [
            [
                ["I", "heard", "Lucille", "has", "three", "sons"],
                ["Rumor", "has", "it", "Lucille", "has", "3", "sons", "!"],
            ],
            [["I", "love", "all", "my", "children", "equally"], ["She", "loves", "all", "her", "children", "equally"]],
        ...
        self.assertEqual(bleu_score(candidate, refs, 3, weights=[0.33, 0.33, 0.33]), 0.4901113)
        # nltk.translate.bleu_score.corpus_bleu(refs, candidate, weights=[0.5]*2)
        self.assertEqual(bleu_score(candidate, refs, 2, weights=[0.5, 0.5]), 0.5119535)
        # nltk.translate.bleu_score.corpus_bleu(refs, candidate, weights=[1])
        self.assertEqual(bleu_score(candidate, refs, 1, weights=[1]), 0.5515605)
