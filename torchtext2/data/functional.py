...
def generate_sp_model(filename, vocab_size=20000, model_type="unigram", model_prefix="m_user"):
    ...
    Examples:
        >>> from torchtext2.data.functional import generate_sp_model
        >>> generate_sp_model('test.csv', vocab_size=23456, model_prefix='spm_user')
    """
    torch.ops.torchtext2.generate_sp_model(filename, vocab_size, model_type, model_prefix)
...
def load_sp_model(spm):
    ...
    if isinstance(spm, str):
        return torch.ops.torchtext2.load_sp_model(spm)
    elif isinstance(spm, io.BufferedReader):
        return torch.ops.torchtext2.load_sp_model_string(spm.read())
    else:
        raise TypeError(
...
def sentencepiece_numericalizer(sp_model):
    ...
    Examples:
        >>> from torchtext2.data.functional import sentencepiece_numericalizer
...
def sentencepiece_tokenizer(sp_model):
    ...
    Examples:
        >>> from torchtext2.data.functional import sentencepiece_tokenizer
...
def simple_space_split(iterator):
    ...
    Examples:
        >>> from torchtext2.data.functional import simple_space_split
...
def numericalize_tokens_from_iterator(vocab, iterator, removed_tokens=None):
    ...
    Examples:
        >>> from torchtext2.data.functional import simple_space_split
        >>> from torchtext2.data.functional import numericalize_tokens_from_iterator
...
def filter_wikipedia_xml(text_iterator):
    ...
    Examples:
        >>> from torchtext2.data.functional import filter_wikipedia_xml
...
def to_map_style_dataset(iter_data):
    ...
    Examples:
        ...
        >>> from torchtext2.data import to_map_style_dataset
