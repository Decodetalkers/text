...
import torch
import torchtext2  # noqa: F401
from torch import Tensor
from torch.nn import Module
from torchtext2._torchtext2 import (
    CLIPEncoder as CLIPEncoderPyBind,
    GPT2BPEEncoder as GPT2BPEEncoderPyBind,
    BERTEncoder as BERTEncoderPyBind,
)
from torchtext2._torchtext2 import RegexTokenizer as RegexTokenizerPybind
from torchtext2.data.functional import load_sp_model
...
class SentencePieceTokenizer(Module):
    ...
    Example
        >>> from torchtext2.transforms import SentencePieceTokenizer
...
class VocabTransform(Module):
    ...
    :param vocab: an instance of :class:`torchtext2.vocab.Vocab` class.

    Example:
        >>> import torch
        >>> from torchtext2.vocab import vocab
        >>> from torchtext2.transforms import VocabTransform
...
class LabelToIndex(Module):
    ...
    def __init__(
        ...
        self._label_vocab = Vocab(torch.classes.torchtext2.Vocab(label_names, None))
...
class GPT2BPETokenizer(Module):
    ...
    def __prepare_scriptable__(self):
        ...
        if not self.is_jitable:
            ...
            tokenizer_copy.bpe = torch.classes.torchtext2.GPT2BPEEncoder(
...
class CLIPTokenizer(Module):
    ...
    Example
        >>> from torchtext2.transforms import CLIPTokenizer
    ...
    def __prepare_scriptable__(self):
        ...
        if not self.is_jitable:
            tokenizer_copy = deepcopy(self)
            # Disable caching in script mode
            tokenizer_copy.bpe = torch.classes.torchtext2.CLIPEncoder(
                self.bpe.bpe_encoder_, self.bpe.bpe_merge_ranks_, self.bpe.seperator_, self.bpe.byte_encoder_, False
            )
            return tokenizer_copy
        return self
...
class BERTTokenizer(Module):
    ...
    Example
        >>> from torchtext2.transforms import BERTTokenizer
    ...
    def __prepare_scriptable__(self):
        if not self.is_jitable:
            tokenizer_copy = deepcopy(self)
            tokenizer_copy.bert_model = torch.classes.torchtext2.BERTEncoder(
...
class RegexTokenizer(Module):
    ...
    Example
        Regex tokenization based on ``(patterns, replacements)`` list.
            >>> import torch
            >>> from torchtext2.transforms import RegexTokenizer
    ...
    def __prepare_scriptable__(self):
        ...
        if not self.is_jitable:
            ...
            regex_tokenizer_copy.regex_tokenizer = torch.classes.torchtext2.RegexTokenizer(
...
class MaskTransform(torch.nn.Module):
    ...
    Example:
        >>> import torch
        >>> from torchtext2.transforms import MaskTransform
        >>> sample_tokens = [
                ["[BOS]", "a", "b", "c", "d"],
                ["[BOS]", "a", "b", "[PAD]", "[PAD]"]
            ]
