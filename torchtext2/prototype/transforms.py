...
from torchtext2._torchtext2 import RegexTokenizer as RegexTokenizerPybind, SentencePiece as SentencePiecePybind
...
def basic_english_normalize():
    ...
    Examples:
        >>> import torch
        >>> from torchtext2.experimental.transforms import basic_english_normalize
...
class BasicEnglishNormalize(nn.Module):
    ...
    Args:
        regex_tokenizer (torch.classes.torchtext2.RegexTokenizer or torchtext2._torchtext2.RegexTokenizer): a cpp regex tokenizer object.
    """
    ...
    def __prepare_scriptable__(self):
        ...
        regex_tokenizer = torch.classes.torchtext2.RegexTokenizer(
...
def load_sp_model(sp_model):
    ...
    Examples:
        >>> from torchtext2.experimental.transforms import load_sp_model
        >>> sp_model = load_sp_model("m_user.model")
        >>> sp_model = load_sp_model(open("m_user.model", 'rb'))

    Note: We also provide several pretrained sentencepiece models. The model was trained with torchtext2.datasets.WikiText103,
        torchtext2.datasets.EnWik9 and BookCorpus. Both BPE and unigram methods were used to train the model (for more
    ...
    Examples:
        >>> from torchtext2.experimental.transforms import PRETRAINED_SP_MODEL
        >>> sp_model_path = torchtext2.utils.download_from_url(PRETRAINED_SP_MODEL['text_unigram_25000'])
...
def sentencepiece_tokenizer(sp_model):
    ...
    Examples:
        >>> import torch
        >>> from torchtext2.experimental.transforms import sentencepiece_tokenizer
...
class SentencePieceTokenizer(nn.Module):
    ...
    def __prepare_scriptable__(self):
        torchbind_spm = torch.classes.torchtext2.SentencePiece(self.sp_model._return_content())
...
def sentencepiece_processor(sp_model):
    ...
    Examples:
        >>> import torch
        >>> from torchtext2.experimental.transforms import sentencepiece_processor
...
class VocabTransform(nn.Module):
    ...
    Args:
        vocab: an instance of torchtext2.vocab.Vocab class.

    Example:
        >>> import torch
        >>> from torchtext2.vocab import vocab_from_file_object
        >>> f = open('vocab.txt', 'r')
...
class VectorTransform(nn.Module):
    ...
    Args:
        vector: an instance of torchtext2.experimental.vectors.Vectors class.

    Example:
        >>> import torch
        >>> from torchtext2.experimental.vectors import FastText
